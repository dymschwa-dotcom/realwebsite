...
class ManageUsersController extends Controller
{
    ...
    {
        $pageTitle = 'Email Unverified Brands';
    ...
    {
        $pageTitle = 'KYC Unverified Brands';
    ...
    {
        $pageTitle = 'Email Verified Brands';
    ...
    {
        $pageTitle = 'Mobile Unverified Brands';
    ...
    {
        $pageTitle = 'Mobile Verified Brands';
    ...
    public function detail($id)
    {
        ...
        $pageTitle = 'Brand Detail - ' . $user->username;

        $totalDeposit = Deposit::where('user_id', $user->id)->successful()->sum('amount');
        $totalTransaction = Transaction::where('user_id', $user->id)->count();

        $campaigns = Campaign::where('status', '!=', Status::CAMPAIGN_INCOMPLETE)->where('user_id', $user->id)->select(['id', 'status', 'start_date', 'end_date'])->get();
        $widget['total_campaign']    = (clone $campaigns)->count();
        $widget['approved_campaign'] = (clone $campaigns)->where('status', Status::CAMPAIGN_APPROVED)->count();
        $widget['rejected_campaign'] = (clone $campaigns)->where('status', Status::CAMPAIGN_REJECTED)->count();
        $widget['running_campaign']  = (clone $campaigns)->where('status', Status::CAMPAIGN_APPROVED)->where('start_date', '<=', now())->where('end_date', '>=', now())->count();

        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalTransaction', 'countries','widget'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $notify[] = ['success', 'Brand details updated successfully'];
        return back()->withNotify($notify);
    }
