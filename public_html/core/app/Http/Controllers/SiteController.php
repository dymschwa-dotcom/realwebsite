...
class SiteController extends Controller {
    ...
    public function categories() {
        ...
        $categories = Category::active()->withCount('influencers')->withSum('influencers', 'rating')->orderBy('name')->paginate(getPaginate());
        $sections   = Page::where('tempname', activeTemplate())->where('slug', 'categories')->first();
        return view('Template::categories', compact('categories', 'pageTitle', 'sections'));
    }
    ...
    public function attachmentDownload($attachment, $conversation_id, $type) {
        if ($type == 'campaign') {
            CampaignConversation::where('id', $conversation_id)->firstOrFail();
