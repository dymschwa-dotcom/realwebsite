...
<script>
    ...
    if (user == 'user') {
        ...
        var authRoute = '{{ route('user.add.device.token') }}';
    } else {
        var authUser = '{{ auth()->guard('influencer')->user() ? true : false }}';
        var authRoute = '{{ route('influencer.add.device.token') }}';
    }
    var authenticated = authUser;
