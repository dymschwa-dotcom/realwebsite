                        ...
                        <div class="card-body">
                            <div class="influencer__profile">
                                <div class="influencer__profile-thumb">
                                    <img src="{{ getImage(getFilePath('brand') . '/' . $user->image) }}"
                                         alt="@lang('image')">
                                </div>
                ...
                <div class="col-xxl-8 col-xl-7 col-lg-12">
                    <div class="row gy-3">
                        <div class="col-xl-6 col-sm-6">
                            <x-widget style="2" link="{{ route('admin.deposit.list', $user->id) }}" title="Deposits"
                                      icon="las la-wallet" value="{{ showAmount($totalDeposit) }}" color="success"
                                      type="2" />
                        </div>

                        <div class="col-xl-6 col-sm-6">
                            <x-widget style="2" link="{{ route('admin.report.transaction', $user->id) }}"
                                      title="Transactions" icon="las la-exchange-alt" value="{{ $totalTransaction }}"
                                      color="warning" type="2" />
                        </div>

                        <div class="col-xl-6 col-sm-6">
                            <x-widget value="{{ getAmount($widget['total_campaign']) }}" title="Total Campaign"
                                      style="2" link="{{ route('admin.campaign.index') }}?search={{ $user->username }}"
                                      icon="lab la-wpforms" color="cyan" />
                        </div>
                        <div class="col-xl-6 col-sm-6">
                            <x-widget value="{{ getAmount($widget['running_campaign']) }}" title="Running Campaign"
                                      style="2" link="{{ route('admin.campaign.approved') }}?search={{ $user->username }}"
                                      icon="las la-hourglass-half" color="teal" />
                        </div>
                        <div class="col-xl-6 col-sm-6">
                            <x-widget value="{{ getAmount($widget['approved_campaign']) }}" title="Approved Campaign"
                                      style="2" link="{{ route('admin.campaign.approved') }}?search={{ $user->username }}"
                                      icon="las la-check-circle" color="success" />
                        </div>
                        <div class="col-xl-6 col-sm-6">
                            <x-widget value="{{ getAmount($widget['rejected_campaign']) }}" title="Rejected Campaign"
                                      style="2" link="{{ route('admin.campaign.rejected') }}?search={{ $user->username }}"
                                      icon="las la-times-circle" color="danger" />
                        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.users.status', $user->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        @if ($user->status == Status::USER_ACTIVE)
                            <h6 class="mb-2">@lang('If you ban this brand he/she won\'t able to access his/her dashboard.')</h6>
                            <div class="form-group">
                        ...
                        @else
                            ...
                            <p>{{ $user->ban_reason }}</p>
                            <h4 class="text-center mt-3">@lang('Are you sure to unban this brand?')</h4>
                        @endif
                    </div>
                    <div class="modal-footer">
                        @if ($user->status == Status::USER_ACTIVE)
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.users.login', $user->id) }}" target="_blank" class="btn btn-sm btn-outline--primary"><i
           class="las la-sign-in-alt"></i>@lang('Login as Brand')</a>
@endpush
