...
@section('content')
    @guest
        <div class="container @if ($layout == 'frontend') py-120 @else py-60 @endif">
            @if ($layout == 'frontend')
                <div class="alert alert-warning">
                    <p>@lang('The contact message has been sent successfully and a new ticket has been added. please copy the URL because the administrator will reply to this ticket shortly.') </p>
                </div>
            @endif
                        ...
                        @auth
                            @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->user)
                                <button class="btn btn--danger close-button btn--sm confirmationBtn"
                                        ...
                                        type="button"><i class="fas fa-lg fa-times-circle"></i>
                                </button>
                            @endif
                        @endauth
                    </div>

                    <div class="card-body {{ auth()->check() ? 'p-3' : '' }}">
                            ...
                            <div class="row justify-content-between">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <textarea class="form-control form--control" name="message" rows="4" required placeholder="@lang('Reply here...')">{{ old('message') }}</textarea>
                                    </div>
                                </div>

                                <div class="col-md-9">
                                    <button class="btn btn-dark btn--sm addAttachment my-2" type="button"> <i
                        ...
                        @if ($message->admin_id == 0)
                            <div class="support-card mb-3">
                                ...
                                <div class="support-card__body">
                                    <p class="support-card__body-text mb-0">
                                        {{ $message->message }}
                                    </p>

                                    @if ($message->attachments->count() > 0)
                                        <ul class="list list--row support-card__list justify-content-center justify-content-md-start flex-wrap gap-3 mt-2">
                        ...
                        @else
                            <div class="support-card mb-3">
                                ...
                                <div class="support-card__body">
                                    ...
                                    @if ($message->attachments->count() > 0)
                                        <ul
                                            class="list list--row support-card__list justify-content-center justify-content-md-start flex-wrap gap-3 mt-2">
                                            @foreach ($message->attachments as $k => $image)
                                                <li>
                                                    <a class="support-card__file"
                                                       href="{{ route('ticket.download', encrypt($image->id)) }}">
        ...
        (function($) {
            ...
            $('.addAttachment').on('click', function() {
                ...
                $(".fileUploadsContainer").append(`
                    <div class="col-lg-{{ auth()->check() ? 6 : 4 }} col-md-12 removeFileInput">
                        <div class="form-group">
