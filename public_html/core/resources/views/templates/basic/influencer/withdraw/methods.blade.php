...
@section('content')
    <div class="brand-dashboard-header d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <div class="brand-dashboard-header__left">
            <h5>{{ @$pageTitle }}</h5>
            <p class="fs-14">@lang('Select the withdraw option to withdraw the balance.')</p>
        </div>
        <div class="brand-dashboard-header__right">
            <a class="btn btn--base btn--sm outline" type="button" href="{{ route('influencer.withdraw.history') }}"><i class="fa-solid fa-money-bill"></i> @lang('Withdraw History')</a>
        </div>
    </div>
    <form class="withdraw-form" action="{{ route('influencer.withdraw.money') }}" method="post">
        @csrf
                    ...
                    <div class="payment-system-list is-scrollable gateway-option-list">
                        @foreach ($withdrawMethod as $data)
                            <label class="payment-item @if ($loop->index > 4) d-none @endif gateway-option" for="{{ titleToKey($data->name) }}">
                                ...
                                <input class="payment-item__radio gateway-input" id="{{ titleToKey($data->name) }}" name="method_code" data-gateway='@json($data)' data-min-amount="{{ showAmount($data->min_limit) }}" data-max-amount="{{ showAmount($data->max_limit) }}" type="radio" value="{{ $data->id }}" hidden @checked(old('method_code', $loop->first) == $data->id)>
                            </label>
                        @endforeach
                        ...
                        <div class="deposit-info">
                            <div class="deposit-info__title">
                                <p class="text has-icon">@lang('Processing Charge')
                                    <span class="proccessing-fee-info" data-bs-toggle="tooltip" title="@lang('Processing charge for withdraw method')"><i
